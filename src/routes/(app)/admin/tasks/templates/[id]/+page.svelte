...
<svelte:head>
	<title>Edit Template - {data.template.name} - TeamTime</title>
</svelte:head>
...
<div class="p-4 lg:p-8 max-w-3xl mx-auto">
	<div class="mb-6">
		...
		<p class="text-gray-600 mt-1">{data.template.name}</p>
	</div>
	...
	{#if data.rules.length > 0}
		<div class="card mb-6">
			<div class="card-header flex justify-between items-center">
				<h2 class="font-semibold">Assignment Rules Using This Template</h2>
				<a href="/admin/tasks/rules/new?templateId={data.template.id}" class="text-sm text-primary-600 hover:text-primary-700">
					+ Add Rule
				</a>
			</div>
			<div class="divide-y divide-gray-200">
				{#each data.rules as rule}
					<a href="/admin/tasks/rules/{rule.id}" class="px-4 py-3 flex items-center justify-between hover:bg-gray-50">
						<div>
						...
						<span class="px-2 py-1 text-xs font-medium rounded-full {rule.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}">
			...
			<div class="card-header flex justify-between items-center">
				...
				<label class="flex items-center">
					<input
						type="checkbox"
						name="isActive"
						checked={data.template.isActive}
				...
				<div>
					...
					<select id="locationId" name="locationId" class="input">
						...
						{#each data.locations as location}
							<option value={location.id} selected={data.template.locationId === location.id}>
								{location.name}
							</option>
						{/each}
					</select>
			...
			<div class="card-body space-y-4">
				<label class="flex items-center">
					<input
						type="checkbox"
						name="photoRequired"
						checked={data.template.photoRequired}
						class="w-4 h-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
					/>
				...
				<label class="flex items-center">
					<input
						type="checkbox"
						name="notesRequired"
						checked={data.template.notesRequired}
						class="w-4 h-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
					/>
			...
			<div class="card-body space-y-4">
				...
				<div>
					...
					<select id="triggerEvent" name="triggerEvent" class="input">
						...
						<option value="clock_in" selected={data.template.triggerEvent === 'clock_in'}>Clock In</option>
						<option value="clock_out" selected={data.template.triggerEvent === 'clock_out'}>Clock Out</option>
						<option value="first_clock_in" selected={data.template.triggerEvent === 'first_clock_in'}>First Clock-In of Day</option>
						<option value="last_clock_out" selected={data.template.triggerEvent === 'last_clock_out'}>Last Clock-Out of Day</option>
					</select>
				</div>
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						...
						<input
							...
							value={data.template.triggerConditions?.timeWindowStart || ''}
							class="input"
						/>
					</div>
					<div>
						...
						<input
							...
							id="timeWindowEnd"
							name="timeWindowEnd"
							value={data.template.triggerConditions?.timeWindowEnd || ''}
							class="input"
						/>
					</div>
				</div>
		...
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
			<div class="bg-white rounded-lg max-w-md w-full p-6">
				...
				<p class="text-gray-600 mb-4">
					Are you sure you want to delete "{data.template.name}"?
					{#if data.rules.length > 0}
						This will also delete {data.rules.length} associated rule{data.rules.length !== 1 ? 's' : ''}.
					{/if}
