...
│   │   │   ├── utils/           # Server utilities (timezone.ts)
...
| `src/lib/server/utils/timezone.ts` | Pacific timezone utilities |
...
TeamTime operates in **Pacific Time (America/Los_Angeles)**. The server runs in UTC, but all business logic uses Pacific timezone.

```typescript
// Parse datetime-local input as Pacific, return UTC Date
parsePacificDatetime(datetimeLocal: string): Date

// Get start and end of day in Pacific (as UTC)
getPacificDayBounds(date?: Date): { start: Date, end: Date }

// Get date parts (year, month, day, hour, etc.) in Pacific
getPacificDateParts(date: Date): { year, month, day, hour, minute, dayOfWeek }

// Format dates for display in Pacific
toPacificDateString(date: Date): string      // "Dec 13, 2025"
toPacificTimeString(date: Date): string      // "9:30 AM"
toPacificDateTimeString(date: Date): string  // Full format

// Create UTC Date from Pacific components
createPacificDateTime(dateStr: string, hour: number, minute?: number): Date

// Get Pacific offset (handles DST)
getPacificOffset(date: Date): number  // -8 (PST) or -7 (PDT)
```

**Key Behaviors:**
- All `datetime-local` form inputs are interpreted as Pacific
- Task due dates use Pacific end-of-day (11:59 PM Pacific)
- "First clock-in of day" triggers at Pacific midnight
- Report date filters use Pacific day boundaries
- AI context shows all dates in Pacific format
...
*Documentation generated by comprehensive codebase analysis. Last updated: December 13, 2025 (Timezone audit complete)*
