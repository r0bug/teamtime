...
describe('generatePartNumber — MDDYY format', () => {
	...
	it('handles double-digit months without padding', async () => {
		mockInsert.mockResolvedValue([{ lastNumber: 3 }]);
		const sku = await generatePartNumber('v1', { now: new Date(Date.UTC(2026, 11, 5, 12, 0, 0)) });
		// month=12 (already 2 digits), day=05 (padded), year=26, serial=003
		expect(sku).toBe('SR120526003');
	});
});
